...
process.stdin.on('data', async chunk => {
  ...
  const componentDirectoryMain = resolve(componentDirectory, 'main') // 当前文件下面 在新增加一个 main目录
  /**
   * vue组件路径
   */
  const componentVueName = resolve(componentDirectoryMain, inputName + '.vue')
  /**
   * 入口文件路径
   */
  ...
  if (hasComponentDirectory) {
    ...
    await dotExistDirectoryCreate(componentDirectoryMain)
