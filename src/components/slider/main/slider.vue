<template>
  <div class="luck-slider" ref="slider">
    <div class="luck-slider-wrapper" ref="warpper"
         :class="direction === 'vertical' ? 'luck-slider-wrapper-vertical' : ''"
         :style="dragStyleObject"
    >
      <div class="luck-slider-item" v-if="loop" :style="itemHeight" v-html="lastItem"></div>
      <slot></slot>
      <div class="luck-slider-item" v-if="loop" :style="itemHeight" v-html="firstItem"></div>
    </div>
    <div class="luck-slider-pagination" v-if="itemsArr.length > 1 && showPagination"
         :class="direction === 'vertical' ? 'luck-slider-pagination-vertical' : ''">
            <span class="luck-slider-pagination-item"
                  v-for="(t, i) in itemNums" :key="i"
                  :class="paginationIndex === i ? 'luck-slider-pagination-item-active': ''"
...
export default {
  ...
  methods: {
    init () {
      ...
      this.itemsArr = this.$children.filter(item => item.$options.name === 'luck-slider-item')
    ...
    touchEndHandler () {
      ...
      if (touches.moveTag === 2) {
        ...
        if ((timeDiff > 250 && Math.abs(moveOffset) <= warpperSize * 0.5) || this.itemsArr.length <= 1 || unloopDrag) {
